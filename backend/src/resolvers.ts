...
import { products as productsStore, warehouses as warehousesStore } from './data';
...
export const resolvers = {
  Query: {
    ...
    warehouses: (): Warehouse[] => {
      return warehouses.slice().sort((a, b) => a.code.localeCompare(b.code));
    },
  ...
  Mutation: {
    updateDemand: (_: unknown, args: { id: string; demand: number }): Product => {
    ...
    transferStock: (
      ...
      const src = products.find(p => p.id === id && p.warehouse === from);
      if (!src) {
        const any = products.find(p => p.id === id);
        if (!any) {
          throw new GraphQLError(`Product with id ${id} not found in inventory`);
        }
